...
class GlobalTripletModel(BaseNN):
    ...
    def _prepare_data(self):
        ...
        print_logging(
            "Author emb shape = %s, word emb shape = %s" % (str(self.author_emb.shape), str(self.word_emb.shape)))
    ...
    def _create_placeholders(self):
        self.train_words_q = tf.placeholder(tf.int32, shape=[None, None],
                                            name='train_words_q')  # batch_size * max_q_len
        self.train_words_q_weights = tf.placeholder(tf.float32, shape=[None, None],
                                                    name='words_idf')  # batch_size * max_q_len
        self.pos_train_words_d = tf.placeholder(tf.int32, shape=[None, None],
                                                name='pos_train_words_d')  # batch_size * max_d_len
        self.pos_train_person_words_d = tf.placeholder(tf.int32, shape=[None, None, None],
                                                       name='pos_train_person_words_d')  # batch_size * max_paper_num * per_max_d_len
        self.neg_train_words_d = tf.placeholder(tf.int32, shape=[None, None],
                                                name='neg_train_words_d')  # batch_size * max_d_len
        self.neg_train_person_words_d = tf.placeholder(tf.int32, shape=[None, None, None],
                                                       name='neg_train_person_words_d')  # batch_size * max_paper_num * per_max_d_len

        self.train_authors_q = tf.placeholder(tf.int32, shape=[None, None],
                                              name='train_authors_q')  # batch_size * max_q_len
        self.train_authors_q_weights = tf.placeholder(tf.float32, shape=[None, None],
                                                      name='authors_idf')  # batch_size * max_q_len
        self.pos_train_authors_d = tf.placeholder(tf.int32, shape=[None, None],
                                                  name='pos_train_authors_d')  # batch_size * max_d_len
        self.pos_train_person_authors_d = tf.placeholder(tf.int32, shape=[None, None, None],
                                                         name='pos_train_person_authors_d')  # batch_size * max_paper_num * per_max_d_len
        self.neg_train_authors_d = tf.placeholder(tf.int32, shape=[None, None],
                                                  name='neg_train_authors_d')  # batch_size * max_d_len
        self.neg_train_person_authors_d = tf.placeholder(tf.int32, shape=[None, None, None],
                                                         name='neg_train_person_authors_d')  # batch_size * max_paper_num * per_max_d_len
        ...
        self.test_authors_q = tf.placeholder(tf.int32, shape=[None, None],
                                             name='test_authors_q')  # batch_size * max_q_len
        ...
        self.test_words_q_weights = tf.placeholder(tf.float32, shape=[None, None],
                                                   name='test_words_idf')  # batch_size * max_q_len
        self.test_authors_d = tf.placeholder(tf.int32, shape=[None, None],
                                             name='test_authors_d')  # batch_size * max_d_len
        self.test_words_d = tf.placeholder(tf.int32, shape=[None, None], name='test_words_d')  # batch_size * max_d_len
        self.test_person_authors_d = tf.placeholder(tf.int32, shape=[None, None, None],
                                                    name='test_person_authors_d')  # batch_size * max_paper_num * per_max_d_len
        self.test_person_words_d = tf.placeholder(tf.int32, shape=[None, None, None],
                                                  name='test_person_words_d')  # batch_size * max_paper_num * per_max_d_len
        self.test_paper_num = tf.placeholder(tf.int32, shape=[None], name='test_paper_num')  # [batch_size]

        self.input_pos_feature = tf.placeholder(tf.float32, shape=[None, None],
                                                name='input_pos_feature')  # ins_num * feature_size
        self.input_neg_feature = tf.placeholder(tf.float32, shape=[None, None],
                                                name='input_neg_feature')  # ins_num * feature_size

        self.keep_prob = tf.placeholder(tf.float32, name='keep_prob')

    def _create_variables(self):
        self.c1 = tf.Variable(tf.constant(self.word_emb, dtype='float32', shape=[self.word_num, settings.EMB_DIM]),
                              name='c1', trainable=False)
        ...
        self.a1 = tf.Variable(tf.constant(self.author_emb, dtype='float32', shape=[self.author_num, settings.EMB_DIM]),
                              name='a1', trainable=False)
    ...
    def get_interaction_vec(self, q_embed, d_embed, person_d_embed, q_weights, paper_num):
        ...
        person_weights = tf.reshape(tf.tile(q_weights, [1, person_kde_shape[1]]),
                                    [person_kde_shape[0], person_kde_shape[1], person_kde_shape[2]])
    ...
    def new_model_bags(self, q_word, q_author, d_word, d_author, word_embeddings, author_embeddings, q_word_weight,
                       q_author_weight):
    ...
    def new_model_ins(self, q_word, q_author, person_d_word, person_d_author, word_embeddings, author_embeddings,
                      q_word_weight, q_author_weight, paper_num):
        q_word_embed = tf.nn.embedding_lookup(word_embeddings, q_word)
        q_author_embed = tf.nn.embedding_lookup(author_embeddings, q_author)
        ...
        person_word_att = tf.tanh(
            tf.nn.bias_add(tf.einsum('ijk,kl->ijl', person_word_sim_vec, self.word_W), self.word_b))
        person_author_att = tf.tanh(
            tf.nn.bias_add(tf.einsum('ijk,kl->ijl', person_author_sim_vec, self.author_W), self.author_b))
    ...
    def _create_loss(self):
        ...
        pos_sim_vec_bags = self.new_model_bags(self.train_words_q, self.train_authors_q, self.pos_train_words_d,
                                               self.pos_train_authors_d,
                                               self.word_embeddings, self.author_embeddings, self.train_words_q_weights,
                                               self.train_authors_q_weights)

        pos_sim_vec_ins = self.new_model_ins(self.train_words_q, self.train_authors_q, self.pos_train_person_words_d,
                                             self.pos_train_person_authors_d,
                                             self.word_embeddings, self.author_embeddings, self.train_words_q_weights,
                                             self.train_authors_q_weights, self.pos_paper_num)

        pos_sim_vec = tf.concat([pos_sim_vec_ins, pos_sim_vec_bags], 1)

        neg_sim_vec_bags = self.new_model_bags(self.train_words_q, self.train_authors_q, self.neg_train_words_d,
                                               self.neg_train_authors_d,
                                               self.word_embeddings, self.author_embeddings, self.train_words_q_weights,
                                               self.train_authors_q_weights)

        neg_sim_vec_ins = self.new_model_ins(self.train_words_q, self.train_authors_q, self.neg_train_person_words_d,
                                             self.neg_train_person_authors_d,
                                             self.word_embeddings, self.author_embeddings, self.train_words_q_weights,
                                             self.train_authors_q_weights, self.neg_paper_num)
        ...
        test_sim_vec_bags = self.new_model_bags(self.test_words_q, self.test_authors_q, self.test_words_d,
                                                self.test_authors_d,
                                                self.word_embeddings, self.author_embeddings, self.test_words_q_weights,
                                                self.test_authors_q_weights)

        test_sim_vec_ins = self.new_model_ins(self.test_words_q, self.test_authors_q, self.test_person_words_d,
                                              self.test_person_authors_d,
                                              self.word_embeddings, self.author_embeddings, self.test_words_q_weights,
                                              self.test_authors_q_weights, self.test_paper_num)
    ...
    def _create_optimizer(self):
        ...
        self.optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate, epsilon=self.epsilon).minimize(
            self.loss)
        self.fine_tune_optimizer = tf.train.AdamOptimizer(learning_rate=self.fine_tune_lr,
    ...
    def get_top_feature(self, test_authors_q, test_authors_q_weights, test_words_q, test_words_q_weights,
        ...
        score, fea_vec = self.sess.run([self.test_score, self.feature_vec],
                                       feed_dict={self.test_words_q: np.array(test_words_q),
                                                  self.test_words_q_weights: np.array(test_words_q_weights),
                                                  self.test_authors_q: np.array(test_authors_q),
                                                  self.test_authors_q_weights: np.array(test_authors_q_weights),
                                                  self.test_words_d: np.array(test_words_d),
                                                  self.test_authors_d: np.array(test_authors_d),
    ...
    def predict(self, test_authors_q_list, test_authors_q_weights_list, test_words_q_list, test_words_q_weights_list,
                test_authors_d_list, test_words_d_list, test_person_authors_d_list, test_person_words_d_list,
                test_paper_num_list):
        ...
        for batch_index in np.arange(len(test_words_q_list)):
            predictions = self._predict_a_batch(
                test_authors_q_list[batch_index], test_authors_q_weights_list[batch_index],
                test_words_q_list[batch_index], test_words_q_weights_list[batch_index],
                test_authors_d_list[batch_index], test_words_d_list[batch_index],
                test_person_authors_d_list[batch_index], test_person_words_d_list[batch_index],
                test_paper_num_list[batch_index])
    ...
    def predict_cl(self, test_authors_q_list, test_authors_q_weights_list, test_words_q_list, test_words_q_weights_list,
                   test_authors_d_list, test_words_d_list, test_person_authors_d_list, test_person_words_d_list,
                   test_paper_num_list, test_id_list):
    ...
    def train(self, train_words_q, train_words_q_weights,
        ...
        return self.sess.run([self.loss, self.l2_loss, self.optimizer, self.pos_score, self.neg_score],
                             feed_dict={self.train_words_q: np.array(train_words_q),
                                        self.train_words_q_weights: np.array(train_words_q_weights),
                                        self.pos_train_words_d: np.array(pos_train_words_d),
                                        self.pos_train_person_words_d: np.array(pos_train_person_words_d),
                                        self.neg_train_words_d: np.array(neg_train_words_d),
                                        self.neg_train_person_words_d: np.array(neg_train_person_words_d),
                                        self.train_authors_q: np.array(train_authors_q),
                                        self.train_authors_q_weights: np.array(train_authors_q_weights),
                                        self.pos_train_authors_d: np.array(pos_train_authors_d),
                                        self.pos_train_person_authors_d: np.array(pos_train_person_authors_d),
                                        self.neg_train_authors_d: np.array(neg_train_authors_d),
                                        self.neg_train_person_authors_d: np.array(neg_train_person_authors_d),
                                        self.pos_paper_num: np.array(pos_papers),
                                        self.neg_paper_num: np.array(neg_papers),
                                        self.keep_prob: 1
                                        })
...
if __name__ == '__main__':
    ...
    train_data = gen_train_data_profile.generate_train_data("train_author_pub_index_profile.json",
                                                            "train_author_pub_index_test.json", settings.TRAIN_SCALE,
                                                            settings.NEG_SAMPLE)

    test_data = gen_train_data_profile.generate_test_data("test_author_pub_index_profile.json",
                                                          "test_author_pub_index_test.json", settings.TEST_SCALE,
                                                          settings.TEST_SAMPLE)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for epoch_count in range(settings.EPOCHES):
            ...
            for batch_index in batch_indexes:
                ...
                train_loss, l2_loss, _, pos_score, neg_score = model.train(TR_new_paper_word_ids[batch_index],
                                                                           TR_new_paper_word_idfs[batch_index], \
                                                                           TR_new_pos_person_word_ids[batch_index],
                                                                           TR_new_pos_per_person_word_ids[batch_index],
                                                                           TR_new_pos_person_papers[batch_index], \
                                                                           TR_neg_person_word_ids[batch_index],
                                                                           TR_neg_per_person_word_ids[batch_index],
                                                                           TR_neg_person_papers[batch_index], \
                                                                           TR_new_paper_author_ids[batch_index],
                                                                           TR_new_paper_author_idfs[batch_index], \
                                                                           TR_new_pos_person_author_ids[batch_index],
                                                                           TR_new_pos_per_person_author_ids[
                                                                               batch_index], \
                                                                           TR_neg_person_author_ids[batch_index],
                                                                           TR_neg_per_person_author_ids[batch_index])
            ...
            if epoch_count % settings.VERBOSE == 0:
                ...
                predictions = model.predict(
                    TE_new_paper_author_ids, TE_new_paper_author_idfs, TE_new_paper_word_ids, TE_new_paper_word_idfs, \
                    TE_person_author_ids, TE_person_word_ids, TE_per_person_author_ids, TE_per_person_word_ids,
                    TE_person_papers)
                # print("ed")
                # print(predictions)
                ...
                print(
                    "hits@{} = {} mrr: {} test_time: {}".format(top_k, ratio_top_k, mrr, round(time() - test_begin, 2)))
